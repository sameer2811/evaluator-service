...
import { BYTE_HEADER_SIZE } from "../util/constants";
...
export default function decodeBufferStream(buffer: Buffer): dockerStreamOutput {
  ...
  while (offset < buffer.length) {
    ...
    if (streamOutput === 1) {
      ...
      output.stdOut = output.stdOut + buffer.toString("utf-8", offset, offset + chunkLength);
    } else if (streamOutput === 2) {
      // this means this is the error stream

      // Reading of the error stream of specified chunk length is done
      output.stderr = output.stderr + buffer.toString("utf-8", offset, offset + chunkLength);
    } else {
    }
